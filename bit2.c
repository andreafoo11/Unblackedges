...
/*
 * struct of Bit2_T that will hold the width, and height of the bit 
 * array and a pointer to the one dimensional bit array.
 */
struct Bit2_T {
    int width;
    int height;
...
Bit2_T Bit2_new(int width, int height)
{
    ...
    /*calculate length of the large 1D bit array to give the appearence 
...
{
    /* Makes sure that the pointer to the array is not NULL, if it is, 
     * CRE */
    assert(array_2D != NULL);
...
{
    /* Makes sure that the pointer to the array is not NULL and x and 
     * y are within bounds otherwise, CRE
     */
    assert(array_2D != NULL);
                    ...
                    Bit2_T array_2D, int curr, void *cl), void *cl)
{
    ...
    * CRE */
    ...
    /*loops through the entire 2D array column by column and calls 
    * function on each element in the 2D array */
